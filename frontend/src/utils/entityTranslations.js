...
// 标准化字符串（去除空格、连字符，转大写；保留下划线以匹配关系类型如 HAS_ROLE）
const normalize = (str) => {
  if (!str) return ''
...
export const translateEntityType = (type) => {
  ...
  if (entityTypeMap[key]) return entityTypeMap[key]
  // 回退：也尝试去除下划线后匹配（兼容蛇形命名如 INDUSTRY_ASSOCIATION）
  const keyNoUnderscore = key.replace(/_/g, '')
  if (entityTypeMap[keyNoUnderscore]) return entityTypeMap[keyNoUnderscore]
  return type
}
...
export const translateRelationType = (type) => {
  if (!type) return '关联'
  const key = normalize(type)
  return relationTypeMap[key] || type
}
