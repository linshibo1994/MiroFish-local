...
def test_graphiti_llm_wrapper_normalizes_deepseek_node_aliases(monkeypatch):
    from app.services import zep_graphiti_impl
    from graphiti_core.llm_client.config import LLMConfig
    from pydantic import BaseModel
...
def test_graphiti_llm_wrapper_normalizes_deepseek_edge_aliases(monkeypatch):
    from app.services import zep_graphiti_impl
    from graphiti_core.llm_client.config import LLMConfig
    from pydantic import BaseModel
    ...
    async def run_call():
        return await wrapped.generate_response([], response_model=FakeEdges)
...
def test_graphiti_llm_wrapper_parses_fenced_json_without_response_format(monkeypatch):
    from app.services import zep_graphiti_impl
    from graphiti_core.llm_client.config import LLMConfig
    ...
    class FakeLLM:
        ...
        async def generate_response(self, *args, **kwargs):
            error = RuntimeError("This response_format type is unavailable now")
            error.status_code = 400
            raise error
...
def test_graphiti_llm_wrapper_retries_empty_json_fallback_response(monkeypatch):
    from app.services import zep_graphiti_impl
    from graphiti_core.llm_client.config import LLMConfig
    ...
    assert asyncio.run(run_call()) == {"ok": True}
    assert fake_llm.client.chat.completions.calls == 2
