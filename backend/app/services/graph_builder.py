...
3. 实体类型不做固定数量限制；尽量完整覆盖政府/监管、单位、机构、企业/品牌、媒体、组织/协会、意见领袖/网红、社区、公众、主配角、网民/个人、以及事件角色（受害人、嫌疑人、目击者、家属等）等关键具体相关主体。
4. 核心人物必须优先抽取：受害人/被害人、嫌疑人/犯罪嫌疑人、被告人、当事人、主角/配角、死者、伤者、亲属、证人、律师等，只要文档文本块出现并与事件有关，就不能因为其不一定发声而忽略。
5. 若同一人物同时出现全名和匿名/代称（如“许国利/许某”“来惠利/来某/来女士/被害人”），优先使用文本中出现的全名作为实体名称，匿名称谓放入属性或摘要。
6. 实体关系应从文档文本块中的事实出发。对于背景、广告、推荐或相似案例段落中的实体，需判断其是否与事件主线存在实质性关联；如果有关联也应抽取。
7. 如果某个名称出现在文本块中但关系不明确，仍建议抽取并保留，避免遗漏潜在关键实体。
8. 不要把本约束中的类别词、规则文本或示例当作实体；实体事实只能来自“文档文本块”。
9. 纯地点/位置/地址（如省市区、街道、小区）不应作为 Person 类型实体节点，但可以作为 Location 类型或事件背景属性保留。案发地、居住地等地点信息应抽取为 Location 节点。（/）
10. 若名称指向组织（如政府机构、学校、医院、公司、媒体等），按组织主体抽取；若名称只是商场/小区/地址，即使与事件有关也不要抽为 Agent 候选实体。
11. 小红书、微博、抖音、豆瓣、知乎等是媒体/社交平台，必须保留为 MediaPlatform/SocialMediaPlatform/Media 等平台或媒体主体，不能归为 Person。
12. 与事件主题明确相关的实体应全部入图；仅作为背景噪音或广告推荐出现的无关实体可以忽略，但需以当前文档文本块的实际内容为准，不要机械套用例。
