...
3. 实体类型不做固定数量限制；必须尽量完整覆盖政府/监管、单位、机构、企业/品牌、媒体、组织/协会、意见领袖/网红、社区、公众、主配角、网民/个人等关键具体相关主体。
4. 核心人物必须优先抽取：受害人/被害人、嫌疑人/犯罪嫌疑人、被告人、当事人、主角/配角、死者、伤者、亲属、证人、律师等，只要文档文本块出现并与事件有关，就不能因为其不一定发声而忽略。
5. 若同一人物同时出现全名和匿名/代称（如“许国利/许某”“来惠利/来某/来女士/被害人”），优先使用文本中出现的全名作为实体名称，匿名称谓放入属性或摘要；不要只创建“许某”“来某”这类弱实体。
6. 实体关系必须能从文档文本块中的事实支撑；只有背景、广告、推荐、相似案例或无关段落里的实体不要抽取。
7. 如果某个名称虽然出现在文本块中，但无法说明它和事件或推演方向的关系，必须忽略。
8. 不要把本约束中的类别词、规则文本或示例当作实体；实体事实只能来自“文档文本块”。
9. 严禁抽取纯地点、位置、地址、城市、地区、场所作为实体节点；省市区、街道、小区、商圈、商场、案发地、居住地、化粪池等只能作为事件背景或主体属性，绝不能抽成 Person/个人实体。
...
12. 例如“张雪机车事件”应保留张雪、张雪机车、法国车手瓦伦丁·德比斯、WSBK、820RR-RS等真实相关主体；网易游戏、阴阳师等无关实体即使出现在材料杂讯中也不要入图。
