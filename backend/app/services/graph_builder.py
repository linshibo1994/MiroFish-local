...
2. 实体类型不做固定数量限制；必须尽量完整覆盖政府/监管、单位、机构、企业/品牌、媒体、组织/协会、意见领袖/网红、社区、公众、主配角、网民/个人等关键具体相关主体。
...
6. 严禁抽取纯地点、位置、地址、城市、地区、场所作为实体节点；省市区、街道、小区、商圈、商场、案发地、居住地、化粪池等只能作为事件背景或主体属性，绝不能抽成 Person/个人实体。
7. 若名称指向可发声组织（如政府机构、学校、医院、公司、媒体等），按组织主体抽取；若名称只是商场/小区/地址，即使与事件有关也不要抽为 Agent 候选实体。
8. 小红书、微博、抖音、豆瓣、知乎等是媒体/社交平台，必须归为 MediaPlatform/SocialMediaPlatform/Media 等平台或媒体主体，不能归为 Person。
9. 例如“张雪机车事件”应保留张雪、张雪机车、法国车手瓦伦丁·德比斯、WSBK、820RR-RS等真实相关主体；网易游戏、阴阳师等无关实体即使出现在材料杂讯中也不要入图。
